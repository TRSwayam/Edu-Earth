...
type ElectricBorderProps = PropsWithChildren<{
  ...
  className?: string;
  style?: CSSProperties;
  optimize?: boolean; // disables heavy effects on small screens, reduced motion, or when offscreen
}>;
...
const ElectricBorder: React.FC<ElectricBorderProps> = ({
  ...
  // Observe viewport visibility (pause heavy effects when off-screen)
  useEffect(() => {
    if (!rootRef.current) return;
  ...
  // Respect prefers-reduced-motion and small screens
  useEffect(() => {
  ...
  const effectsActive = !optimize ? true : inView && !reducedMotion && !isSmallScreen;
  ...
  return (
    <div ref={rootRef} className={'relative isolate ' + (className ?? '')} style={style}>
      ...
      <div className="absolute inset-0 pointer-events-none" style={inheritRadius}>
        ...
        {effectsActive && <div className="absolute inset-0 box-border" style={glow1Style} />}
        {effectsActive && <div className="absolute inset-0 box-border" style={glow2Style} />}
        {effectsActive && <div className="absolute inset-0" style={bgGlowStyle} />}
      </div>
